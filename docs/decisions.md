...
2026-09-21 — Direction A, the field. BUILT on `build/03-direction-a` — awaiting the founder's notes.
What it is: Lando Norris's light world, toned to a firm. A contour field — iso-lines over slow noise, marching squares on a 2D canvas — drawn live behind the whole page in ink at 22%; it leans away from the cursor on a laptop and drifts with the scroll on a phone. In the hero the construction mark is a hill in the terrain: the lines bend around it. The headline's lines slide up out of a clipped box on load; the read's three columns rise in as long pills; the four functions sit at the compass points of the mark with its outer circle drawn in the four tones; A few steps ahead on teal with the field showing through in paper; the three terms as circles rising; the story split; Programs on aqua. No smooth-scroll library — the wheel is the browser's. Onest and IBM Plex Mono, self-hosted; rounded kept.
Built in `../Bidaya-a` (a worktree) while the founder's other session works on B. Rules broken on the founder's licence: type (Onest / IBM Plex Mono); the construction mark used as a drawing, and its outer circle in four colours in the four-functions chapter (docs/05 says one colour). The read's column names are the deck's words promoted, as in C. Method change kept from C: a headed-browser wheel check (40 wheel steps, count of frames over 34ms) runs before the founder sees it — 0 long frames; phones draw the field on alternate frames with a coarser grid. Measured: Lighthouse mobile 97 / 100 / 100 / 92.
