...
2026-09-21 — Direction C, the method. BUILT on `build/03-direction-c` — awaiting the founder's notes.
What it is: WeEvolveIT's scroll-through-the-process on a light page. A stream of ink dots behind the page that gathers into what each chapter is about (three circles for the read, four columns for the functions, the mark after them) and loosens between; three chapters that turn the page sideways as the wheel moves — the read's three columns, the four functions in their four colours, the three terms — each with a `01 / 03` counter and a bracketed label in mono; a scroll-progress mark in the corner. Space Grotesk and Space Mono, self-hosted. Rounded shapes kept. On the phone the sideways chapters are swipe rows.
Built in a second copy of the repository (`../Bidaya-c`, a git worktree) so it could be made while the founder's other session edits B in the main folder. Rules broken on the founder's licence: type (Space Grotesk / Space Mono in place of Readex Pro); "the read" column names Covered / Needs building / Yours to do set as headlines — they are the deck's words from "The read — Covered, needs building, yours to do", promoted, not written. Small text on the teal slide sits on a paper strip (ink on teal is 4.3:1, under the floor). Measured: Lighthouse mobile 99 / 96→100 / 100 / 92; 60fps at both widths; the stream is one path per frame and starts after the page is idle.
